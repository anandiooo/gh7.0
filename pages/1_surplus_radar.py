...
with st.container(border=True):
    ...
    status_columns[0].info(weather_label, icon=":material/partly_cloudy_day:")
...
if not radar.risk.has_data:
    ...
    for index, (complete, title_key, help_key, page, icon) in enumerate(setup_steps):
        status_icon = ":material/check_circle:" if complete else ":material/radio_button_unchecked:"
        status = t("radar.setup_done") if complete else t("radar.setup_pending")
        step_columns = st.columns([1.6, 3, 1.6], vertical_alignment="center")
        step_columns[0].markdown(f"**{status_icon} {status} — {t(title_key)}**")
...
with st.container(border=True):
    ...
    figure.add_trace(
        go.Scatter(
            ...
            mode="lines+markers",
            fill="tozeroy",
            fillcolor="rgba(47,139,58,0.15)",
            line=dict(color="#2F8B3A", width=4),
            marker=dict(size=10, color="#2F8B3A", line=dict(color="#F2EFE8", width=2)),
        )
    )
    figure.add_trace(
        go.Scatter(
            ...
            mode="lines+markers",
            line=dict(color="#A8E629", width=4),
            marker=dict(size=10, color="#A8E629", line=dict(color="#1A2E1A", width=2)),
        )
    )
    figure.add_trace(
        go.Scatter(
            ...
            mode="lines+markers",
            line=dict(color="#F2DB1D", width=4, dash="dot"),
            marker=dict(
                size=10,
                color="#F2DB1D",
                line=dict(color="#1A2E1A", width=2),
            ),
        )
    )
    figure.add_vline(
    ...
    figure.update_layout(
        ...
        plot_bgcolor="#F2EFE8",
        font=dict(color="#1A2E1A", family="Inter, Outfit, sans-serif", weight="bold"),
        xaxis=dict(showgrid=False, tickformat="%d %b %Y"),
        yaxis=dict(gridcolor="rgba(31,110,42,0.14)", zeroline=False),
    )
    st.plotly_chart(
        figure,
        width="stretch",
