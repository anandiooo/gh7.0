...
def render_outcome(outcome: AnalysisOutcome) -> None:
    ...
    st.markdown(f"### {t('analysis.allocation_table')}")
            ...
            [
                {
                    ...
                    t("analysis.reason"): ", ".join(
                        t(f"allocation_reason.{code.value.lower()}") for code in item.reason_codes
                    ),
                }
    ...
    with detail_columns[0]:
        st.markdown(f"### {t('analysis.unallocated_batches')}")
    ...
    with detail_columns[1]:
        st.markdown(f"### {t('analysis.unmet_demands')}")
...
if st.button(t("action.run_analysis"), type="primary", key="analysis_run_button"):
    try:
...
st.divider()
st.markdown(f"## {t('scenario.title')}")
...
if use_demand and buyer_contexts:
    selected_buyer = st.selectbox(
        t("scenario.existing_buyer"), buyer_contexts, format_func=lambda item: item.buyer_name
    )
    demand_columns = st.columns(3)
    scenario_quantity = demand_columns[0].number_input(
        t("demand.quantity_kg"), min_value=0.0, value=1000.0, step=100.0
    )
    scenario_deadline = demand_columns[1].date_input(
        t("demand.deadline"), value=base.analysis.horizon_end, key="scenario_deadline"
    )
    scenario_priority = demand_columns[2].selectbox(
...
if use_capacity:
    ...
    capacity_date = capacity_columns[0].selectbox(
        t("common.date"), [item.date for item in base.analysis.daily_capacity]
    )
    current_capacity = next(
        item.quantity_kg for item in base.analysis.daily_capacity if item.date == capacity_date
    )
    capacity_value = capacity_columns[1].number_input(
...
if st.button(
    ...
    try:
        ...
        capacity_overrides = (
            (TemporaryCapacityOverride(date=capacity_date, effective_capacity_kg=capacity_value),)
            if use_capacity
            else ()
        )
    ...
    except (MimpiTaniError, PydanticValidationError) as error:
        st.error(
            user_safe_error_message(error)
            if isinstance(error, MimpiTaniError)
        ...
        st.success(t("scenario.completed"), icon="✅")
